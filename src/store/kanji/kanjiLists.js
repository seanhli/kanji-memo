...
export const kanjiSlice = createSlice({
  ...
  reducers: {
    ...
    createKanjiLists: (state, action) => {
      ...
      state.Qs["beginner"] = [...action.payload[0]].sort(
        (a, b) => 0.5 - Math.random()
      );
      state.Qs["intermediate"] = [...action.payload[1]].sort(
        (a, b) => 0.5 - Math.random()
      );
      state.Qs["advanced"] = [...action.payload[2]].sort(
        (a, b) => 0.5 - Math.random()
      );
    ...
    nextKanji: (state, action) => {
      ...
      if (action && action.payload in state.Qs)
        if (state.Qs[action.payload].length < 40) {
          ...
          state.Qs[action.payload] = [...state.kanjiLists[action.payload]].sort(
            (a, b) => 0.5 - Math.random()
          );
        }
